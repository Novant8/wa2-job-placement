...
import {isDwellingAddress, isEmailAddress, isPhoneAddress} from "../types/address.ts";
...
export default  function ProfessionaInfo (){
    ...
    useEffect(() => {
        ...
        const professionalIdNumber = professionalId ? Number(professionalId) : undefined;
        setLoading(true);
    ...
    const notDoneOffers = jobOffers.filter(offer => offer.offerStatus !== "Done");
    const doneOffers = jobOffers.filter(offer => offer.offerStatus === "Done");

    return (
       <>
           <Row className="pb-3" style={{borderBottom: "dotted grey 1px"}}>
                 <h1>{professional.contactInfo.name + "\t"+professional.contactInfo.surname}</h1>
           </Row>
                ...
                <Row className="pb-3" style={{borderBottom: "dotted grey 1px"}}>
                    {professional.contactInfo?.addresses.map(address => {
                        if (isEmailAddress(address)) {
                            ...
                            return (
                                <Col sm={6} key={address.id}>
                                    <b>Telephone</b>: <br/>{address.phoneNumber}
                                </Col>
                            );
                        }else if (isDwellingAddress(address)){
                            return (
                                <Col sm={6} key={address.id}>
                                    <b>Address</b>: <br/>{address.street + ", " + address.city + ", " + address.district + address.country}
                                </Col>
                            );
                        }
                    })}
           ...
           <Row className="mt-3" style={{justifyContent:"center",borderBottom: "dotted grey 1px"}} >
               <Col sm ={6}>
                   <h3>Information</h3>
               </Col>
               <Row className="mt-3 pb-3">
                   ...
                   <Col sm={3} >
                       <b> SSN  </b>: <br/> {professional.contactInfo.ssn ?  professional.contactInfo.ssn : "N/A"}
                   </Col>
                   <Col sm={3} >
                       <b> Employment State  </b>: <br/> {professional.employmentState }
                   </Col>
                   <Col sm={3} >
           ...
           <Row className="mt-3 pb-3" style={{borderBottom: "dotted grey 1px"}}>
               ...
               {notDoneOffers.length > 0 ? (
                           ...
                           <Row key={offer.id} xs={12} className="mb-4">
                               <Card>
                                   <Card.Body>
                                       ...
                                       <Button variant="primary" onClick={() => navigate(`/crm/RecruiterJobOffer/${offer.id}`, {replace :true })}>
                                           View
                                       </Button>
                                   </Card.Body>
                               </Card>
                   ...
                   <p >
                       No Active job offers available.
                   </p>
               )}
           </Row>
           <Row className="mt-3 pb-3">
               ...
               {doneOffers.length > 0 ? (
                   ...
                   <p >
                       No completed job offers available.
                   </p>
               )}
           </Row>
       </>
