...
export function getAddressType(address: Address): AddressType | undefined {
  if (isEmailAddress(address)) return "EMAIL";
  if (isPhoneAddress(address)) return "TELEPHONE";
  if (isDwellingAddress(address)) return "ADDRESS";
}
...
export function isDwellingAddress(
  address: Address,
): address is DwellingAddress {
  return ["street", "city", "district", "country"].every((prop) =>
    address.hasOwnProperty(prop),
  );
}
